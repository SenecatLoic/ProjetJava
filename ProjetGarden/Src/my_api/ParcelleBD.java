...
public class ParcelleBD implements Parcelle{
    ...
    public Iterator<Action> getAllActions(){
        ...
        PreparedStatement requete = null;
        int idTmp=0;
        ArrayList<Integer> idMere = new ArrayList<Integer>();
        idMere.add(new Integer(this.idParcelle));

        //requête mise en forme de string
        String stringReq = "select idAction,idLegume from Action where idParcelle=?";

        //on récupère tout les id des parcelles mère
        while( (idTmp = this.getMother().getID()) != this.getRoot().getID()){
            idMere.add(new Integer(idTmp));
        }
    ...
    public void addAction(Action action){
        ...
        PreparedStatement requete = null;
        int idAction = 0;
        ActionSolBD actionSol = null;
        ActionLegumeBD actionLegume = null;

        if (action instanceof ActionLegumeBD) {
            actionLegume = (ActionLegumeBD) action;
            idAction = actionLegume.getId();
        } else{
            actionSol = (ActionSolBD) action;
            idAction = actionSol.getId();
        }
